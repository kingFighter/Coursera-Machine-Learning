...
for i = 1 : m
    ...
    select = zeros(1, size(a3, 1));
    select(y(i)) = 1;
    J = J + sum(-select * log(a3) - (1 - select) * log(1 - a3));
    theta3 = a3 - select';
    theta2 = Theta2'* theta3 .* sigmoidGradient([1;Theta1 * X(i,:)']);
    Theta1_grad =  Theta1_grad + theta2(2:end) * X(i,:);
    Theta2_grad = Theta2_grad + theta3 * a2';
end;
